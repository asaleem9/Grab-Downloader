...
политика конфиденциальности Grabа проста: мы ничего не собираем и не храним о
тебе. то, что ты делаешь, — это исключительно твоё дело, а не наше или чьё-либо
ещё.

эти условия применяются только при использовании официального инстанса Grabа.
...
при использовании функции сохранения, Grabу может понадобиться проксировать
...
на неизменённом и официальном инстансе Grabа **все данные туннеля шифруются
...
если исходный код Grabа не изменён.

медиаданные из туннелей нигде не хранятся/кэшируются. всё обрабатывается в
реальном времени, даже при ремуксинге и транскодировании. туннели Grabа
...
число активных пользователей Grabа, полностью анонимно. никакая
...
plausible](https://{env.PLAUSIBLE_HOST}/), который использует Grab.
